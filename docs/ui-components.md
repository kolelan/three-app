...
| Элемент | Назначение |
|---------|------------|
| `#mobile-weapon-bar` | Иконки оружия 1–7 вверху экрана |
| `#mobile-joystick` | Виртуальный стик → `KeyW` / `KeyA` / `KeyS` / `KeyD` |
| `#mb-strafe-left` / `right` | Шаг вбок (`Q` / `E`), над джойстиком |
| `#mobile-look-zone` | Правая зона — поворот и прицел |
| `#mb-zoom` | Дальний бой (G), вкл/выкл |
| `#mb-third-person` | Вид сзади (V), скрыт в режиме G |
| `#mb-wheel-in` / `out` | Колесо «на себя» / «от себя» (FOV или дистанция 3-го лица) |
| `#mb-autoaim` | Автонаведение: скрывает прицел, ведёт огонь по ближайшему видимому врагу в радиусе 50 |
| `#mb-fire` | ЛКМ / огнемёт (удержание) |
| `#mb-alt` | ПКМ |
| `#mb-help` | Подсказка ЛКМ/ПКМ для текущего оружия |

На мобильных скрываются `#controls`, `#info`, панель оружия в `#stats`; HUD справа (`body.mobile-game`).
